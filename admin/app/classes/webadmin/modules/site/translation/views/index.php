...
<div class="ui clearing basic segment title-segment">
    <h3 class="ui left floated orange header translation-header">
...
<div class="ui segment">
    ...
    <select class="ui dropdown" id="lang">
        <?php foreach ($langs as $k => $v) : ?>
            <option value="<?= $this->e($k) ?>" <?= $k == $lang ? 'selected' : '' ?>><?= $this->e($v) ?></option>
        <?php endforeach ?>
    </select>
...
$("#store").on('click', function (e) {
    e.preventDefault();
    $.post('<?= $url('ptranslation/store') ?>?lang=<?= $this->e($lang) ?>').always(function () {
