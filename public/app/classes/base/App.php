...
class App
{
    ...
    public function defaults(): array
    {
        ...
        return [
            ...
            "UPLOADHASHKEY"                => 'Place-a-random-hash-key-here'
        ];
    }
    ...
    public function schema(bool $refresh = false): void
    {
        ...
        if (!$refresh && $cached = $cache->get($key)) {
            $dbc->setSchema(unserialize($cached));
        } else {
            $cache->getSet(
                $key,
                function () use ($dbc) {
                    $dbc->parseSchema();
                    return serialize($dbc->getSchema());
                }
            );
        }
    }
    ...
    public function files(): Files
    {
        ...
        $files = new Files(
            ...
            $this->config->getString('UPLOADHASHKEY')
        );
            ...
            array_filter([
                in_array('file', $locations) ?
                    new StreamHandler(
                        ...
                        date('Y') . '/' . date('m.d') . '.log',
                        $this->config->getBool('DEBUG') ? Logger::DEBUG : Logger::INFO
                    ) :
                    null,
                ...
                in_array('syslog', $locations) ?
                    new SyslogHandler(
                        ...
                        LOG_USER,
                        $this->config->getString('DEBUG') ? Logger::DEBUG : Logger::INFO
                    ) :
                    null
            ])
        );
