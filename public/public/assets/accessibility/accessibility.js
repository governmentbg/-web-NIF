...
const accessibility = async (opts) => {
    ...
    if (!parentSelector || !navSelector || !mainSelector) {
        throw new Error(
            '"parentSelector" or "navSelector" or "mainSelector" are not set'
        );
    }
    ...
    const hideSelectorsElements = hideSelectors.flatMap((e) => [
    ...
    if (!links) {
        console.warn(
            `ACCESSIBILITY-TOOL: No link elements found with selector ${linkSelector}`
        );
    }
    const increaseFontSize = (e) => {
        ...
        localStorage.setItem(
            ...
            Number(body.classList.contains("font-resize"))
        );
        document
            .querySelector(".a11y__item--increase")
            .querySelector("span").textContent = body.classList.contains(
            "font-resize"
        )
        ...
        for (let i = 0; i < document.styleSheets.length; i++) {
            if (document.styleSheets.item(i)) {
                textOnly.v
                    ...
                    : (document.styleSheets.item(i).disabled =
                          !document.styleSheets.item(i).disabled);
            }
        }
    ...
    const reset = () => {
        ...
        document
            .querySelector(".a11y__item--increase")
            .querySelector("span").textContent = translations[lng]?.fontUp;
    };
    const getSettings = () => {
        Number(localStorage.getItem("accesibility.fontSize")) &&
            increaseFontSize();
        Number(localStorage.getItem("accesibility.dyslectic")) &&
    ...
    const controlPanelNav = ef(
        ...
        ef(
            ...
            ef(
                ...
                ef(
                    "h2",
                    { class: "a11y-tools__title" },
                    translations[lng].changeContrast
                ),
                ef(
                    "div",
            ...
            ef(
                ...
                ef(
                    "p",
                    {},
                    translations[lng].logoText ?? "© Information Services Jsc"
                )
            )
        )
    );
